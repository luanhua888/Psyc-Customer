...
const ModalForgotPassword = forwardRef((props, ref) => {
  ...
  const handleResendCode = async () => {
    ...
    try {
      if (email) {
        const responseResendCode = await userService.registerResendCode(email);
        ...
        {errorMessages.message && (
          <div
            className={`flex justify-center items-center mb-3 font-medium ${
              errorMessages.isError ? "text-[#ff7010]" : "text-[#ff7010]"
            }`}
          >
            {errorMessages.message}
          </div>
        ...
        <Formik
          ...
          validate={(values) => {
            ...
            if (actionType[ACTION_TYPE.RESEND_CODE]) {
              ...
              // mật khẩu phải có ít nhất 6 ký tự, 1 chữ hoa, 1 chữ thường, 1 số
              else if (
